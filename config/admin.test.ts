...
describe("estAdmin", () => {
  it("refuse tout le monde quand la variable n'est pas configurée", () => {
    ...
    expect(estAdmin("fondateur@clearto.fr")).toBe(false);
  });
  ...
  it("autorise un email listé", () => {
    process.env.ADMIN_EMAILS = "fondateur@clearto.fr";
    expect(estAdmin("fondateur@clearto.fr")).toBe(true);
  });

  it("ignore la casse et les espaces autour", () => {
    process.env.ADMIN_EMAILS = " Fondateur@Clearto.fr , associe@clearto.fr ";
    expect(estAdmin("FONDATEUR@clearto.fr")).toBe(true);
    expect(estAdmin("  associe@clearto.fr ")).toBe(true);
  });
  ...
  it("ne se laisse pas avoir par un sous-domaine ressemblant", () => {
    process.env.ADMIN_EMAILS = "fondateur@clearto.fr";
    expect(estAdmin("fondateur@clearto.fr.attaquant.com")).toBe(false);
  });
});
